...
private:
    ...
    std::string find_device_by_address(const std::string& address)
    {
        ...
        auto it = std::find_if(devices.begin(), devices.end(),
            [&address](const BluetoothDevice& dev) {
    ...
    bool cmd_list(const std::vector<std::string>& args)
    {
        ...
        {
            std::cout << std::setw(18) << device.address << " "
                      << std::setw(20) << (device.name.empty() ? device.alias : device.name) << " "
                      << std::setw(6) << device.rssi << " "
                      << std::setw(8) << (device.paired ? "Yes" : "No") << " "
                      << std::setw(10) << (device.connected ? "Yes" : "No") << std::endl;
        }
        std::cout << std::endl;
        return true;
    }
    ...
    bool cmd_show(const std::vector<std::string>& args)
    {
        ...
        auto devices = agent_->list();
        auto it = std::find_if(devices.begin(), devices.end(),
            [&target_device](const BluetoothDevice& dev) {
