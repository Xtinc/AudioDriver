...
void WienerFilter::Update(int32_t num_analyzed_frames, ArrayView<const float, kFftSizeBy2Plus1> noise_spectrum,
                          ...
                          ArrayView<const float, kFftSizeBy2Plus1> signal_spectrum)
{
    for (size_t i = 0; i < kFftSizeBy2Plus1; ++i)
    {
        ...
        float prev_tsa = spectrum_prev_process_[i] / (prev_noise_spectrum[i] + 0.0001f) * filter_[i];
        ...
        filter_[i] = std::max(std::min(filter_[i], 1.f), suppression_params_.minimum_attenuating_gain);
    }
    ...
    if (num_analyzed_frames < kShortStartupPhaseBlocks)
    {
        for (size_t i = 0; i < kFftSizeBy2Plus1; ++i)
        {
            ...
            float filter_initial = initial_spectral_estimate_[i] -
                                   suppression_params_.over_subtraction_factor * parametric_noise_spectrum[i];
            filter_initial /= initial_spectral_estimate_[i] + 0.0001f;

            filter_initial = std::max(std::min(filter_initial, 1.f), suppression_params_.minimum_attenuating_gain);

            // Weight the two suppression filters.
            constexpr float kOnyByShortStartupPhaseBlocks = 1.f / kShortStartupPhaseBlocks;
    ...
    std::copy(signal_spectrum.begin(), signal_spectrum.end(), spectrum_prev_process_.begin());
}

float WienerFilter::ComputeOverallScalingFactor(int32_t num_analyzed_frames, float prior_speech_probability,
