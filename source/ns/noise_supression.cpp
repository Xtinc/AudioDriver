...
void NoiseSuppressor::Analyze(ArrayView<const float, kNsFrameSize> audio)
{
    ...
    for (size_t ch = 0; ch < num_channels_; ++ch)
    {
        float energy = ComputeEnergyOfExtendedFrame(audio, channels_[ch]->analyze_analysis_memory);
        if (energy > 0.f)
        {
            zero_frame = false;
            break;
    ...
    for (size_t ch = 0; ch < num_channels_; ++ch)
    {
        ...
        FormExtendedFrame(audio, ch_p->analyze_analysis_memory, extended_frame);
...
void NoiseSuppressor::Process(ArrayView<float, kNsFrameSize> audio)
{
    ...
    {
        // Form an extended frame and apply analysis filter bank windowing.
        FormExtendedFrame(audio, channels_[ch]->process_analysis_memory, filter_bank_states[ch].extended_frame);
    ...
    for (size_t ch = 0; ch < num_channels_; ++ch)
    {
        OverlapAndAdd(filter_bank_states[ch].extended_frame, channels_[ch]->process_synthesis_memory, audio);
    }
    ...
    for (size_t ch = 0; ch < num_channels_; ++ch)
    {
        ...
        {
            audio[j] = std::min(std::max(audio[j], -32768.f), 32767.f);
        }
    }
}
