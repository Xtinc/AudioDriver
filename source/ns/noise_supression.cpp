...
static constexpr std::array<float, 96> kBlocks160w256FirstHalf = {
    0.00000000f, 0.01636173f, 0.03271908f, 0.04906767f, 0.06540313f, 0.08172107f, 0.09801714f, 0.11428696f, 0.13052619f,
    0.14673047f, 0.16289547f, 0.17901686f, 0.19509032f, 0.21111155f, 0.22707626f, 0.24298018f, 0.25881905f, 0.27458862f,
    0.29028468f, 0.30590302f, 0.32143947f, 0.33688985f, 0.35225005f, 0.36751594f, 0.38268343f, 0.39774847f, 0.41270703f,
    0.42755509f, 0.44228869f, 0.45690388f, 0.47139674f, 0.48576339f, 0.50000000f, 0.51410274f, 0.52806785f, 0.54189158f,
    0.55557023f, 0.56910015f, 0.58247770f, 0.59569930f, 0.60876143f, 0.62166057f, 0.63439328f, 0.64695615f, 0.65934582f,
    0.67155895f, 0.68359230f, 0.69544264f, 0.70710678f, 0.71858162f, 0.72986407f, 0.74095113f, 0.75183981f, 0.76252720f,
    0.77301045f, 0.78328675f, 0.79335334f, 0.80320753f, 0.81284668f, 0.82226822f, 0.83146961f, 0.84044840f, 0.84920218f,
    0.85772861f, 0.86602540f, 0.87409034f, 0.88192126f, 0.88951608f, 0.89687274f, 0.90398929f, 0.91086382f, 0.91749450f,
    0.92387953f, 0.93001722f, 0.93590593f, 0.94154407f, 0.94693013f, 0.95206268f, 0.95694034f, 0.96156180f, 0.96592583f,
    0.97003125f, 0.97387698f, 0.97746197f, 0.98078528f, 0.98384601f, 0.98664333f, 0.98917651f, 0.99144486f, 0.99344778f,
    0.99518473f, 0.99665524f, 0.99785892f, 0.99879546f, 0.99946459f, 0.99986614f};

// Applies the filterbank window to a buffer.
static void ApplyFilterBankWindow(ArrayView<float, kFftSize> x)
{
...
static void FormExtendedFrame(ArrayView<const float, kNsFrameSize> frame,
                              ...
                              ArrayView<float, kFftSize> extended_frame)
{
    ...
    std::copy(frame.begin(), frame.end(), extended_frame.begin() + old_data.size());
    std::copy(extended_frame.end() - old_data.size(), extended_frame.end(), old_data.begin());
}

// Uses overlap-and-add to produce an output frame.
static void OverlapAndAdd(ArrayView<const float, kFftSize> extended_frame,
                          ArrayView<float, kOverlapSize> overlap_memory, ArrayView<float, kNsFrameSize> output_frame)
{
    for (size_t i = 0; i < kOverlapSize; ++i)
    {
        output_frame[i] = overlap_memory[i] + extended_frame[i];
    }
    std::copy(extended_frame.begin() + kOverlapSize, extended_frame.begin() + kNsFrameSize,
              output_frame.begin() + kOverlapSize);
    std::copy(extended_frame.begin() + kNsFrameSize, extended_frame.end(), overlap_memory.begin());
}

// Produces a delayed frame.
static void DelaySignal(ArrayView<const float, kNsFrameSize> frame,
                        ...
                        ArrayView<float, kNsFrameSize> delayed_frame)
{
    ...
    std::copy(frame.begin(), frame.begin() + kSamplesFromFrame, delayed_frame.begin() + delay_buffer.size());

    std::copy(frame.begin() + kSamplesFromFrame, frame.end(), delay_buffer.begin());
}

// Computes the energy of an extended frame.
static float ComputeEnergyOfExtendedFrame(ArrayView<const float, kFftSize> x)
{
    float energy = 0.f;
    for (float x_k : x)
    {
...
static float ComputeEnergyOfExtendedFrame(ArrayView<const float, kNsFrameSize> frame,
                                          ArrayView<float, kFftSize - kNsFrameSize> old_data)
{
    float energy = 0.f;
...
static void ComputeMagnitudeSpectrum(ArrayView<const float, kFftSize> real, ArrayView<const float, kFftSize> imag,
                                     ArrayView<float, kFftSizeBy2Plus1> signal_spectrum)
{
    signal_spectrum[0] = fabsf(real[0]) + 1.f;
    signal_spectrum[kFftSizeBy2Plus1 - 1] = fabsf(real[kFftSizeBy2Plus1 - 1]) + 1.f;

    for (size_t i = 1; i < kFftSizeBy2Plus1 - 1; ++i)
    {
        signal_spectrum[i] = SqrtFastApproximation(real[i] * real[i] + imag[i] * imag[i]) + 1.f;
    }
}

// Compute prior and post SNR.
static void ComputeSnr(ArrayView<const float, kFftSizeBy2Plus1> filter, ArrayView<const float> prev_signal_spectrum,
                       ArrayView<const float> signal_spectrum, ArrayView<const float> prev_noise_spectrum,
                       ArrayView<const float> noise_spectrum, ArrayView<float> prior_snr, ArrayView<float> post_snr)
{
...
static float ComputeUpperBandsGain(float minimum_attenuating_gain, ArrayView<const float, kFftSizeBy2Plus1> filter,
                                   ...
                                   ArrayView<const float, kFftSizeBy2Plus1> signal_spectrum)
{
    ...
    for (size_t i = kFftSizeBy2Plus1 - kNumAvgBins - 1; i < kFftSizeBy2Plus1 - 1; i++)
    {
...
NoiseSuppressor::NoiseSuppressor(int ns_level, size_t num_channels)
    : num_bands_(1), num_channels_(num_channels), suppression_params_(ns_level), channels_(num_channels_)
{
    for (size_t ch = 0; ch < num_channels_; ++ch)
    {
        channels_[ch] = std::make_unique<ChannelState>(suppression_params_, num_bands_);
    }
}

void NoiseSuppressor::AggregateWienerFilters(ArrayView<float, kFftSizeBy2Plus1> filter) const
{
    ArrayView<const float, kFftSizeBy2Plus1> filter0 = channels_[0]->wiener_filter.get_filter();
    std::copy(filter0.begin(), filter0.end(), filter.begin());

    for (size_t ch = 1; ch < num_channels_; ++ch)
    {
        ArrayView<const float, kFftSizeBy2Plus1> filter_ch = channels_[ch]->wiener_filter.get_filter();
...
void NoiseSuppressor::Analyze(const ChannelBuffer<float> &audio)
{
    ...
    for (size_t ch = 0; ch < num_channels_; ++ch)
    {
        ArrayView<const float, kNsFrameSize> y_band0(&audio.bands(ch)[0][0], kNsFrameSize);
        float energy = ComputeEnergyOfExtendedFrame(y_band0, channels_[ch]->analyze_analysis_memory);
        if (energy > 0.f)
        {
            zero_frame = false;
            break;
    ...
    for (size_t ch = 0; ch < num_channels_; ++ch)
    {
        ...
        // presence.
        ch_p->noise_estimator.PreUpdate(num_analyzed_frames_, signal_spectrum, signal_spectral_sum);

        std::array<float, kFftSizeBy2Plus1> post_snr;
        std::array<float, kFftSizeBy2Plus1> prior_snr;
        ComputeSnr(ch_p->wiener_filter.get_filter(), ch_p->prev_analysis_signal_spectrum, signal_spectrum,
                   ch_p->noise_estimator.get_prev_noise_spectrum(), ch_p->noise_estimator.get_noise_spectrum(),
                   prior_snr, post_snr);

        ch_p->speech_probability_estimator.Update(num_analyzed_frames_, prior_snr, post_snr,
        ...
        // method.
        std::copy(signal_spectrum.begin(), signal_spectrum.end(), ch_p->prev_analysis_signal_spectrum.begin());
    }
}

void NoiseSuppressor::Process(ChannelBuffer<float> &audio)
{
    // Select the space for storing data during the processing.
    std::array<FilterBankState, 2> filter_bank_states_stack;
    ArrayView<FilterBankState> filter_bank_states(filter_bank_states_stack.data(), num_channels_);
    std::array<float, 2> upper_band_gains_stack;
    ArrayView<float> upper_band_gains(upper_band_gains_stack.data(), num_channels_);
    std::array<float, 2> energies_before_filtering_stack;
    ArrayView<float> energies_before_filtering(energies_before_filtering_stack.data(), num_channels_);
    std::array<float, 2> gain_adjustments_stack;
    ArrayView<float> gain_adjustments(gain_adjustments_stack.data(), num_channels_);

    // Compute the suppression filters for all channels.
    for (size_t ch = 0; ch < num_channels_; ++ch)
    {
        // Form an extended frame and apply analysis filter bank windowing.
        ArrayView<float, kNsFrameSize> y_band0(&audio.bands(ch)[0][0], kNsFrameSize);

        FormExtendedFrame(y_band0, channels_[ch]->process_analysis_memory, filter_bank_states[ch].extended_frame);

        ApplyFilterBankWindow(filter_bank_states[ch].extended_frame);

        energies_before_filtering[ch] = ComputeEnergyOfExtendedFrame(filter_bank_states[ch].extended_frame);
        ...
        std::array<float, kFftSizeBy2Plus1> signal_spectrum;
        ComputeMagnitudeSpectrum(filter_bank_states[ch].real, filter_bank_states[ch].imag, signal_spectrum);

        // Compute the frequency domain gain filter for noise attenuation.
        channels_[ch]->wiener_filter.Update(num_analyzed_frames_, channels_[ch]->noise_estimator.get_noise_spectrum(),
        ...
        if (num_bands_ > 1)
        {
            ...
            upper_band_gains[ch] = ComputeUpperBandsGain(suppression_params_.minimum_attenuating_gain,
    ...
    for (size_t ch = 0; ch < num_channels_; ++ch)
    {
        fft_.Ifft(filter_bank_states[ch].real, filter_bank_states[ch].imag, filter_bank_states[ch].extended_frame);
    }

    for (size_t ch = 0; ch < num_channels_; ++ch)
    {
        const float energy_after_filtering = ComputeEnergyOfExtendedFrame(filter_bank_states[ch].extended_frame);
        ...
        gain_adjustments[ch] = channels_[ch]->wiener_filter.ComputeOverallScalingFactor(
            num_analyzed_frames_, channels_[ch]->speech_probability_estimator.get_prior_probability(),
    ...
    for (size_t ch = 0; ch < num_channels_; ++ch)
    {
        ...
        {
            filter_bank_states[ch].extended_frame[i] = gain_adjustment * filter_bank_states[ch].extended_frame[i];
        }
    }
    ...
    {
        ArrayView<float, kNsFrameSize> y_band0(&audio.bands(ch)[0][0], kNsFrameSize);
        OverlapAndAdd(filter_bank_states[ch].extended_frame, channels_[ch]->process_synthesis_memory, y_band0);
    }
    ...
    if (num_bands_ > 1)
    {
        ...
        for (size_t ch = 0; ch < num_channels_; ++ch)
        {
            for (size_t b = 1; b < num_bands_; ++b)
            {
                ...
                // the lowest band.
                ArrayView<float, kNsFrameSize> y_band(&audio.bands(ch)[b][0], kNsFrameSize);
...
NoiseSuppressor::ChannelState::ChannelState(const SuppressionParams &suppression_params, size_t num_bands)
    : wiener_filter(suppression_params), noise_estimator(suppression_params),
