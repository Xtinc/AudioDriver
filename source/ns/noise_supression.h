#ifndef AUDIO_PROCESSING_NOISE_SUPPRESSION_H
#define AUDIO_PROCESSING_NOISE_SUPPRESSION_H
...
{
  public:
    NoiseSuppressor(int ns_level, size_t num_channels);
    NoiseSuppressor(const NoiseSuppressor &) = delete;
    NoiseSuppressor &operator=(const NoiseSuppressor &) = delete;

    // Analyses the signal (typically applied before the AEC to avoid analyzing
    // any comfort noise signal).
    void Analyze(ArrayView<const float, kNsFrameSize> audio);

    // Applies noise suppression.
    void Process(ArrayView<float, kNsFrameSize> audio);
