...
static void rdft_256(size_t n, int isgn, float *a, size_t *ip, float *w)
{
    size_t nw, nc;
    float xi;
...
{
    // Initialize (setting (bit_reversal_state_[0] to 0 triggers
    bit_reversal_state_[0] = 0;
    std::array<float, kFftSize> tmp_buffer;
    tmp_buffer.fill(0.f);
    rdft_256(kFftSize, 1, tmp_buffer.data(), bit_reversal_state_.data(), tables_.data());
}

void NrFft::Fft(ArrayView<float, kFftSize> time_data, ArrayView<float, kFftSize> real, ArrayView<float, kFftSize> imag)
{
    rdft_256(kFftSize, 1, time_data.data(), bit_reversal_state_.data(), tables_.data());
...
void NrFft::Ifft(ArrayView<const float> real, ArrayView<const float> imag, ArrayView<float> time_data)
{
    time_data[0] = real[0];
