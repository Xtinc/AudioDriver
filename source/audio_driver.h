...
public:
    ...
    unsigned int fs() const { return dev_fs; }
    unsigned int ps() const { return dev_ps; }
    unsigned int ch() const { return dev_ch; }
    unsigned int max_channels() const { return max_ch; }
    unsigned int min_channels() const { return min_ch; }
...
protected:
    AudioDevice(const std::string &name, bool capture, unsigned int fs, unsigned int ps, unsigned int ch)
        : hw_name(name), is_capture_dev(capture), hstate(STREAM_CLOSED), dev_fs(fs), dev_ps(ps), dev_ch(ch),
          max_ch(0), min_ch(0) {};
...
std::unique_ptr<AudioDevice> make_audio_driver(int type, const AudioDeviceName &name, unsigned int fs, unsigned int ps, unsigned int ch);
